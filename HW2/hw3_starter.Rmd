...
dat <- read.csv("data/contraceptive_use-1.csv")
est <- read.csv("data/est-1.csv") 
plot_cp(dat %>% select(-division_numeric_code), est, iso_code = 4)
plot_cp(dat %>% select(-start_date), est, iso_code = 4)
plot_cp(dat %>% select(-contraceptive_use_modern), est, iso_code = 4)
```
